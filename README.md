...
- **Campos**: se definen en un solo lugar, la constante `CAMPOS` de `src/hoja_vida.js`; la plantilla y la validación del servidor se ajustan solas. Se guardan en la columna `datos` (`jsonb`), así que cambiar la lista no requiere migración.
  ...
  - **Códigos de precio por obra**: la obra empieza sin códigos; con *+ Agregar código* se añade una fila con el **tipo de producto** (texto libre, con sugerencias de las líneas de producto activas), el **código** (`R+R`, `R`, `E` o `F`, los mismos de la ficha de obras) y **notas** opcionales (hasta 300 caracteres). Cada fila necesita el producto (código y notas son opcionales), un producto no se repite dentro de la misma obra y se permiten hasta 20. Se guardan en `datos.obras[].precios` como `[{ producto, codigo, notas }]` y en la ficha se muestran como etiquetas con la nota al lado.
...
- **Seguimientos automáticos (etiquetas de Chatwoot)**: arriba del formulario de tareas, un selector múltiple con las etiquetas de seguimiento de la cuenta de Chatwoot (las que empiezan por `seg_`; se cambia con `CHATWOOT_ETIQUETAS_PREFIJO`). Muestra las que tiene la conversación abierta y, al marcar o quitar una, la app las aplica en la conversación por la API de Chatwoot (`CHATWOOT_API_TOKEN`, solo en el servidor): es lo mismo que hacerlo en Chatwoot, así que el flujo *Crear Seguimiento Etiquetas* crea o elimina el seguimiento y unos segundos después la lista de eventos se actualiza sola. Solo se reemplazan las etiquetas de seguimiento; las demás etiquetas de la conversación no se tocan. Antes de leer o cambiar, el servidor comprueba que la conversación sea del mismo celular. Necesita el id de la conversación, que llega en el contexto de Chatwoot (en la vista directa `/hv/agenda/:celular` se puede pasar `?conversacion=ID`).
...
- Lo que recibe n8n ya viene listo para el nodo de Google Calendar: `titulo` con el formato de la agenda que usaban en Calendar, `1 - Llamar al cliente para ofrecerle cemento - 3232` (prioridad, tarea y línea responsable; `✔` al inicio cuando está hecha), `descripcion` (cliente, celular, responsable, prioridad y notas), `inicio` y `fin` en hora de Bogotá (`2026-09-20T10:30:00-05:00`, una hora de duración) o solo la fecha cuando es de todo el día (`todo_el_dia: true`), `color_id` (vacío para las tareas pendientes, que toman el color del calendario; `10`, verde, para las hechas. La API de Google no tiene color blanco ni transparente para eventos: para que las pendientes se vean en blanco, en Google Calendar de la cuenta maestra se pone el calendario con **color personalizado `#FFFFFF`** (Configuración → el calendario → Color → Personalizado). Las tareas sin hora son eventos de todo el día; como el nodo de n8n no puede quitarle el color a un evento, cuando una tarea vuelve a pendiente la app borra el evento y lo crea de nuevo, igual que cuando cambia de todo el día a con hora; la columna `calendario_color` guarda el color con que quedó cada evento), `correo_responsable` y `google_event_id` para actualizar o eliminar.
...
2. Variables de entorno (ver `app/.env.example`):
   ...
   | `CHATWOOT_API_TOKEN` | Token de acceso de un administrador de Chatwoot; la agenda lo usa para asignar y quitar etiquetas de seguimiento |
   | `CHATWOOT_ACCOUNT_ID` | Id de la cuenta de Chatwoot (por defecto `1`) |
   | `CHATWOOT_API_URL` / `CHATWOOT_ETIQUETAS_PREFIJO` | Opcionales: URL de la API si no es `CHATWOOT_ORIGIN`, y prefijo de las etiquetas que se ofrecen (por defecto `seg_`; vacío = todas) |
