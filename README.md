...
- **Seguimientos automáticos (etiquetas de Chatwoot)**: debajo del formulario, un selector múltiple con las etiquetas de seguimiento de la cuenta de Chatwoot (las que empiezan por `seg_`; se cambia con `CHATWOOT_ETIQUETAS_PREFIJO`). Muestra las que tiene la conversación abierta y, al marcar o quitar una, la app las aplica en la conversación por la API de Chatwoot (`CHATWOOT_API_TOKEN`, solo en el servidor): es lo mismo que hacerlo en Chatwoot, así que el flujo *Crear Seguimiento Etiquetas* crea o elimina el seguimiento y unos segundos después la lista de eventos se actualiza sola. Solo se reemplazan las etiquetas de seguimiento; las demás etiquetas de la conversación no se tocan. Antes de leer o cambiar, el servidor comprueba que la conversación sea del mismo celular. Necesita el id de la conversación, que llega en el contexto de Chatwoot (en la vista directa `/hv/agenda/:celular` se puede pasar `?conversacion=ID`).
